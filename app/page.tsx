...
export default function Home() {
  ...
  async function submit() {
    ...
    try {
      await fetch(endpoint, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ action: 'createRequest', ...form, items, total, submittedAt: new Date().toISOString() }) });
  ...
  return <main><header className="topbar"><div className="brand"><span className="brand-mark">睿</span><div><strong>睿思請示單</strong><small>申請、留存、套版，一次完成</small></div></div><span className="status"><span />系統可用</span></header>
